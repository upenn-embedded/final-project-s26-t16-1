...
int main(void) {
    ...
    while (1) {
        if (UCSR0A & (1 << RXC0)) {
            ...
            // Treat any non-digit as a potential "End of Message" 
            // This is safer than just checking for \n
            else if (index > 0) { 
                rx_buffer[index] = '\0';

                // Use atol to convert string to long
                long adc_val = atol(rx_buffer);

                // Constrain value to 10-bit max (1023) just in case
                if (adc_val > 1023) adc_val = 1023;

                // Update PWM: Scale 10-bit to 8-bit
                OCR0B = (uint8_t)(adc_val >> 2);

                // Feedback LED
                PORTB ^= (1 << PB5);

                index = 0; // Always reset after processing
            }
        }
    }
